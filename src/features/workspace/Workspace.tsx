  ...
  return (
    ...
    <div className="h-screen bg-background font-sans text-foreground selection:bg-orange-200 flex">
      ...
      >
        ...
        {/* Header */}
        <header className="sticky top-0 z-10 bg-background/80 backdrop-blur-md px-8 py-6 border-b border-border/40">
        ...
        <div className="p-8 max-w-[1600px] mx-auto">
          ...
          <div className="flex flex-col items-center justify-start pt-6 min-h-[85vh] mb-12 animate-in slide-in-from-bottom-4 duration-700 snap-start scroll-mt-28">
                ...
                <div className="absolute bottom-3 left-3 right-3 flex justify-between items-end">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="relative" ref={fileMenuRef}>
                      {isFileMenuOpen && (
                        <div className="absolute bottom-full left-0 mb-2 w-40 bg-popover rounded-xl shadow-lg border border-border p-1.5 flex flex-col gap-1 animate-in slide-in-from-bottom-2 duration-200 z-20">
                          <button className="flex items-center gap-2 px-3 py-2 hover:bg-accent rounded-lg text-sm text-muted-foreground hover:text-foreground transition-colors w-full text-left">
                            <FileText size={16} />
                            <span>Documento</span>
                          </button>
                          <button className="flex items-center gap-2 px-3 py-2 hover:bg-accent rounded-lg text-sm text-muted-foreground hover:text-foreground transition-colors w-full text-left">
                      ...
                      <button 
                        ...
                        className="p-2.5 rounded-full text-muted-foreground hover:text-orange-500 hover:bg-accent transition-colors"
                    ...
                    <div className="relative" ref={personalityRef}>
                      {isPersonalityOpen && (
                        <div className="absolute bottom-full left-0 mb-2 w-48 bg-popover rounded-xl shadow-lg border border-border p-1.5 flex flex-col gap-1 animate-in slide-in-from-bottom-2 duration-200 z-20">
                          <div className="px-2 py-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Personalidade</div>
                          {[
                            ...
                            <button 
                              ...
                              className={cn(
                                ...
                                selectedPersonality === p.id ? "bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 font-medium" : "text-muted-foreground hover:bg-accent hover:text-foreground"
                              )}
                            >
                              <span>{p.label}</span>
                      ...
                      <button 
                        ...
                        className={cn(
                          ...
                          selectedPersonality !== 'Empathetic' ? "text-orange-500 bg-orange-50 dark:bg-orange-900/20" : "text-muted-foreground hover:text-orange-500 hover:bg-accent"
                        )}
                  ...
                  <div className="relative" ref={modeMenuRef}>
                    {isModeMenuOpen && (
                      <div className="absolute bottom-full right-0 mb-2 w-64 bg-popover rounded-2xl shadow-xl border border-border p-2 flex flex-col gap-1 animate-in slide-in-from-bottom-2 duration-200 z-20">
                        <button 
                          onClick={() => {
                            setSelectedMode('Advisor');
                            setIsModeMenuOpen(false);
                          }}
                          className="group flex items-start gap-3 p-3 hover:bg-accent rounded-xl text-left transition-colors"
                        >
                          <div className="p-2 bg-orange-100 dark:bg-orange-900/30 rounded-lg text-orange-600 dark:text-orange-400 group-hover:bg-orange-200 dark:group-hover:bg-orange-900/50 transition-colors">
                            <Brain size={18} />
                          </div>
                          <div>
                            <div className="font-medium text-foreground text-sm">Conselheiro</div>
                            <div className="text-xs text-muted-foreground">Orientação empática e reflexão</div>
                          </div>
                        </button>
                        ...
                        >
                          ...
                          <div>
                            <div className="font-medium text-foreground text-sm">Planejador</div>
                            <div className="text-xs text-muted-foreground">Passos acionáveis e organização</div>
                          </div>
                        </button>
                      </div>
                    )}

                    <div className="flex items-center bg-zinc-900 dark:bg-white dark:text-zinc-900 rounded-2xl p-1.5 shadow-lg shadow-zinc-900/20 gap-1 transition-transform active:scale-95">
                      <button 
                        onClick={() => setIsModeMenuOpen(!isModeMenuOpen)}
                        className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-orange-50 dark:text-zinc-900 hover:text-white dark:hover:text-zinc-700 transition-colors"
                      >
                        <span className="tracking-wide">{getModeTranslation(selectedMode)}</span>
                        <ChevronDown size={14} className={cn("text-zinc-400 dark:text-zinc-600 transition-transform duration-200", isModeMenuOpen && "rotate-180")} />
                      </button>
            ...
            <div className="mt-12 animate-bounce duration-2000">
              <svg width="40" height="60" viewBox="0 0 40 60" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-muted-foreground opacity-90">
                <path 
          ...
          <div className="grid grid-cols-1 gap-12 items-start w-full snap-start scroll-mt-28">
            ...
            <div className="w-full space-y-8 min-w-0 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-500">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                  <h3 className="text-2xl font-serif font-semibold text-foreground mb-2">Cadernos Recentes</h3>
                  <div className="flex gap-2">
                    {[
                      ...
                      <button key={filter.id} className="px-4 py-1.5 rounded-full text-sm font-medium bg-card border border-border text-muted-foreground hover:text-foreground hover:border-orange-200 transition-colors">
                        {filter.label}
                      </button>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-8">
                ...
                <div className="h-full">
                  <button
                    onClick={() => setIsModalOpen(true)}
                    className="w-full h-full min-h-[250px] rounded-3xl border-2 border-dashed border-orange-200 dark:border-orange-900/30 flex flex-col items-center justify-center gap-4 hover:bg-orange-50/50 dark:hover:bg-orange-900/10 transition-all"
                  >
                    <div className="w-16 h-16 rounded-full bg-orange-100 dark:bg-orange-900/20 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                      <Plus size={32} className="text-orange-400" />
                    </div>
                    <p className="font-medium text-muted-foreground group-hover:text-orange-400 transition-colors">Criar novo caderno</p>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="w-full animate-in fade-in slide-in-from-bottom-8 duration-700 delay-700">
              ...
              <div className="bg-zinc-900 dark:bg-card rounded-3xl p-8 text-orange-50 relative overflow-hidden min-h-[300px] flex flex-col md:flex-row gap-8">
                ...
                <div className="relative z-10 flex flex-col gap-6 flex-1 max-w-2xl">
                  <div>
                    ...
                    <h4 className="text-3xl font-bold mb-4 leading-snug text-white dark:text-foreground">Estudar para a Prova de História</h4>
                    <p className="text-zinc-400 dark:text-muted-foreground text-base mb-6 leading-relaxed">Revisar os capítulos sobre a Revolução Industrial e fazer os exercícios práticos do final do livro. Focar nos principais eventos e datas.</p>
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-sm text-zinc-400 dark:text-muted-foreground">+1 assistente virtual</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="relative z-10 md:w-1/3 min-w-[300px]">
                  <div className="bg-white/5 rounded-2xl p-6 backdrop-blur-sm border border-white/10 h-full flex flex-col">
                    <div className="flex justify-between items-center mb-6">
                      <h5 className="font-medium text-base text-white dark:text-foreground">Etapas de Estudo</h5>
                      <span className="text-xs bg-white/10 px-2 py-1 rounded-lg text-zinc-300">1/3 Concluído</span>
                    </div>
                    <div className="space-y-4">
                      <div className="flex items-center gap-3 text-sm text-zinc-300 dark:text-muted-foreground group cursor-pointer">
                        ...
                        <span className="line-through text-zinc-500 dark:text-muted-foreground/50 truncate group-hover:text-zinc-300 dark:group-hover:text-foreground transition-colors">Ler Capítulo 4</span>
                      </div>
                      <div className="flex items-center gap-3 text-sm text-zinc-300 dark:text-muted-foreground group cursor-pointer">
                        <div className="w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center text-zinc-900 shrink-0 transition-transform group-hover:scale-110"><CheckCircle size={12} /></div>
                        <span className="line-through text-zinc-500 dark:text-muted-foreground/50 truncate group-hover:text-zinc-300 dark:group-hover:text-foreground transition-colors">Fazer resumo dos tópicos</span>
                      </div>
                      <div className="flex items-center gap-3 text-sm group cursor-pointer">
                        ...
                        <span className="truncate text-orange-100 dark:text-foreground group-hover:text-white transition-colors">Resolver 10 questões</span>
                      </div>
                    </div>
                  </div>
                </div>
